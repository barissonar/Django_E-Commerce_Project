...
def mancategories_get(request,categoriename):
     if request.method == 'POST':
        return checkboxfilter_get(request,categoriename)
     if Categories.objects.filter(slug=categoriename,gender=1,topcategorie__isnull=False).exists():
        subcategorie = Categories.objects.get(slug=categoriename,gender=1,topcategorie__isnull=False)
        products = Products.objects.filter(subcategorie=subcategorie.id,gender=1)
        ...
        newbodyfilter_list = [item.lower() for item in bodyfilter if item is not None]
        bodyfilter = set(newbodyfilter_list)

        if categoriename == subcategorie.slug:
          return render(request, 'men/mangetcategorie.html', {"subcategorie":subcategorie, "subproducts":products, "categoriename":categoriename,"brandfilter":brandfilter,"colorfilter":colorfilter,'bodyfilter':bodyfilter})

     elif Categories.objects.filter(slug=categoriename,gender=1,topcategorie__isnull=True).exists():
             topcategorie = Categories.objects.get(slug=categoriename,gender=1,topcategorie__isnull=True)
             products = Products.objects.filter(topcategorie=topcategorie.id,gender=1)
             ...
             if categoriename == topcategorie.slug:
                 return render(request, 'men/mangetcategorie.html', {"topcategorie":topcategorie, "topproducts":products, "categoriename":categoriename,"brandfilter":brandfilter,"colorfilter":colorfilter,'bodyfilter':bodyfilter})
     else:
    ...
    if Categories.objects.filter(slug=categoriename,gender=1,topcategorie__isnull=False).exists():
        ...
        if brands and colors and bodysizes:
            for brand in brands:
                for color in colors:
                    for bodysize in bodysizes:
                        filterproducts += products.filter(brand__brand__iexact=brand,color__color__iexact=color,bodysize__size__iexact=bodysize).values("id", "slug", "name", "brand__brand","price", "image")

        elif bodysizes and colors and not brands:
             for bodysize in bodysizes:
                 for color in colors:
                     filterproducts += products.filter(bodysize__size__iexact=bodysize,color__color__iexact=color).values("id", "slug", "name", "brand__brand","price", "image")

        elif bodysizes and brands and not colors:
             for bodysize in bodysizes:
                 for brand in brands:
                     filterproducts += products.filter(bodysize__size__iexact=bodysize,brand__brand__iexact=brand).values("id", "slug", "name", "brand__brand","price", "image")

        elif bodysizes and not brands and not colors:
            for bodysize in bodysizes:
                     filterproducts += products.filter(bodysize__size__iexact=bodysize).values("id", "slug", "name", "brand__brand","price", "image")

        elif brands and colors and not bodysizes:
            for brand in brands:
                for color in colors:
                    filterproducts += products.filter(brand__brand__iexact=brand, color__color__iexact=color).values("id", "slug", "name", "brand__brand","price", "image")

        elif brands and not colors and not bodysizes:
            for brand in brands:
                filterproducts += products.filter(brand__brand__iexact=brand).values("id", "slug", "name", "brand__brand", "price","image")

        elif colors and not brands and not bodysizes:
            for color in colors:
                filterproducts += products.filter(color__color__iexact=color).values("id", "slug", "name", "brand__brand", "price", "image")
...
def manproductdetail_get(request,categoriename,producturl):
    product = Products.objects.get(slug=producturl,gender=1)
