...
def childcategories_get(request,categoriename):
    if request.method == 'POST':
        return checkboxfilter_get(request, categoriename)
    if Categories.objects.filter(slug=categoriename, gender=3, topcategorie__isnull=False).exists():
        subcategorie = Categories.objects.get(slug=categoriename, gender=3, topcategorie__isnull=False)
        products = Products.objects.filter(subcategorie=subcategorie.id, gender=3)
        ...
        if categoriename == subcategorie.slug:
            return render(request, 'child/childgetcategorie.html',
    ...
    elif Categories.objects.filter(slug=categoriename, gender=3, topcategorie__isnull=True).exists():
        topcategorie = Categories.objects.get(slug=categoriename, gender=3, topcategorie__isnull=True)
        products = Products.objects.filter(topcategorie=topcategorie.id, gender=3)
...
def childproductdetail_get(request,categoriename,producturl):
    product = Products.objects.get(slug=producturl,gender=3)
